            ...
            if subqestions and type(subqestions) is list:
                ...
                for sub_index, subqestion in enumerate(subqestions):
                    ...
                    if ref == "None" or ref == None:
                        ...
                        extractor_input = {"context": context,"question":sub_q}
                        sub_answer = extractor(extractor_input)
                        subqestion["answer"] = sub_answer
                        subqestion["retrieved"] = context
                        subqestion["rewrited"] = sub_q
                        ...
                        results = retriever.search(sub_qestion_rewrited, topk=topk)
                        recall["subquestion"] = sub_qestion_rewrited
                        recall["retrieved"] = results
                        context = "\n\n".join([item["question"]+"\n"+item["answer"] for item in results])
                        context = context.strip()
                        subqestion["retrieved"] = context
                        ...
                        sub_answer = extractor(extractor_input)
                        subqestion["answer"] = sub_answer
                        subqestion["rewrited"] = sub_qestion_rewrited
                        recalls.append(recall)
